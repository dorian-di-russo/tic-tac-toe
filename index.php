...
<body>
	<form name = "ticktactoe" method = "post" action = "index.php">
		<?php
			for($i = 0; $i <=8; $i++)
			{
				printf('<input type = "text" pattern="[x,o]{1}" id = "ip" name = "box%s" value = "%s" maxlength="1" autocomplete="off">', $i, $box[$i]); // utilisation de pattern pour la validité de l'input
				if ($i == 2 || $i == 5 || $i == 8){
				print("<br>"); 
				}
			}
			...
			{
				print('<input type = "button" name = "newgamebtn" value = "Rejouer" id = "go" onclick = "window.location.href=\'index.php\'">'); 
			}
